...
# averaging microbiome across time points
taxa_avg = taxa_raw %>% 
  group_by(subject) %>% 
...
taxa_leadeigen = cluster_leading_eigen(taxa_graph) # This one produces 
```
...
# Poisson regression: first 3 pcs for first cluster

```{r}
# Get eigentaxa
index_final = which(taxa_avg$subject %in% taxa_merge$subject)
taxa_pois = as.data.frame(cbind(taxa_le_eigen1[index_final,1], 
      taxa_le_eigen1[index_final,2],
      taxa_le_eigen1[index_final,3],
      taxa_merge$pe,
      ifelse(taxa_merge$pe == 0, 0, 1)))
colnames(taxa_pois) = c("taxa_eigen1", "taxa_eigen2", "taxa_eigen3",
                        "pe", "pe_bi")
summary(glm(pe ~ taxa_eigen1, family = "poisson", control = glm.control(maxit = 50),data = taxa_pois))

#require(MASS)
#model_nb = glm.nb(pe ~ taxa_eigen1 + taxa_eigen2 + taxa_eigen3, control = glm.control(maxit = 100), data = taxa_pois)
#exp(model_nb$coefficients)
#summary(glm.nb(pe ~ taxa_eigen1 + taxa_eigen2 + taxa_eigen3, control = glm.control(maxit = 100),data = taxa_pois))
#barplot(table(taxa_pois$pe))
```


# Poisson regression:  first 3 pcs for second cluster

```{r}
# Get eigentaxa
index_final = which(taxa_avg$subject %in% taxa_merge$subject)
taxa_pois = as.data.frame(cbind(taxa_le_eigen2[index_final,1], 
      taxa_le_eigen2[index_final,2],
      taxa_le_eigen2[index_final,3],
      taxa_merge$pe,
      ifelse(taxa_merge$pe == 0, 0, 1)))
colnames(taxa_pois) = c("taxa_eigen1", "taxa_eigen2", "taxa_eigen3",
                        "pe", "pe_bi")
...
# Poisson regression:  first 3 pcs for third cluster
...
# Poisson regression:  first 3 pcs for fourth cluster

```{r}
# # Get eigentaxa
# index_final = which(taxa_avg$subject %in% taxa_merge$subject)
# taxa_pois = as.data.frame(cbind(taxa_le_eigen4[index_final,1], 
#       taxa_le_eigen4[index_final,2],
#       taxa_le_eigen4[index_final,3],
#       taxa_merge$pe,
#       ifelse(taxa_merge$pe == 0, 0, 1)))
# colnames(taxa_pois) = c("taxa_eigen1", "taxa_eigen2", "taxa_eigen3",
#                         "pe", "pe_bi")
...
# Poisson regression:  first 3 pcs for fifth cluster

```{r}
# index_final = which(taxa_avg$subject %in% taxa_merge$subject)
# taxa_pois = as.data.frame(cbind(taxa_le_eigen5[index_final,1], 
#       taxa_le_eigen5[index_final,2],
#       taxa_le_eigen5[index_final,3],
#       taxa_merge$pe,
#       ifelse(taxa_merge$pe == 0, 0, 1)))
# colnames(taxa_pois) = c("taxa_eigen1", "taxa_eigen2", "taxa_eigen3",
#                         "pe", "pe_bi")
